...
import { rsiZone, type EntrySignals } from "@/lib/entry-signals";
...
/**
 * The four entry-timing signals for one watchlist row, expanded under it.
 *
 ...
 * figure carries its unit — the read is the owner's to make, so the panel
 * states the facts and stops.
 */
    ...
    <div className="grid grid-cols-2 gap-x-8 gap-y-5 lg:grid-cols-4">
        ...
        {signals.rangePosition === null ? (
          ...
          <>
            ...
            <p className="num text-xs text-ink-300">
              {symbol}
              {formatAmount(signals.rangeLow)} – {symbol}
              {formatAmount(signals.rangeHigh)}
            </p>
          </>
        )}
      </Tile>
      ...
      <Tile label="Trend">
        <p className="text-base">
          {signals.trend === null ? "—" : TREND_LABEL[signals.trend]}
        </p>
        <p className="num text-xs text-ink-300">
          {signals.vs50 === null
